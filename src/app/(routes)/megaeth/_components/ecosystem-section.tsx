    ...
    <section className={`relative ${sectionSpacing} py-16 sm:py-20`}>
      ...
      <div className="mx-auto grid max-w-5xl gap-6 sm:grid-cols-3">
        {ecosystemItems.map((item) => (
          <div
            key={item.title}
            className="flex flex-row sm:flex-col items-center gap-4  px-3 text-left sm:text-center "
          >
            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-light-100/10 text-aqua-100 p-3">
              <item.Icon className="h-5 w-5" />
            </div>
            <p className="text-heading-4 font-normal leading-relaxed text-light-400">{item.title}</p>
          </div>
        ))}
      </div>
    </section>
